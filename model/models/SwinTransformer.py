...
class SwinTransformer3D(nn.Module):
    ...
    def getAttention(self,layer:int=4,block:int=2):
        assert len(self.layers)>=layer,f"[SwinTransformer] 存在しないレイヤ{layer:02d}が指定されました。最大レイヤ:{len(self.layers):02d}"
        assert len(self.layers[layer-1].blocks)>=block,f"[SwinTransformer] 存在しないブロック{block:02d}が指定されました。最大ブロック:{len(self.layers[layer-1].blocks):02d}"
        
        return torch.Tensor(self.get_submodule(f"layers.{layer-1}.blocks.{block-1}.attn").attn)
